...
if os.path.exists(session_dir) and os.listdir(session_dir):
    try:
        ...
        gar = Garmin()
        gar.login()
        print("✅ Сессия Garmin восстановлена из кэша")
    except Exception as e:
        print(f"⚠️ Сессия не подошла, пробуем вход по паролю: {e}")
        gar = None

if gar is None:
    try:
        print("🔑 Выполняем полный вход по логину/паролю...")
        gar = Garmin(GARMIN_EMAIL, GARMIN_PASSWORD)
        gar.login()
        garth.save(session_dir)
        print("💾 Вход успешен, сессия сохранена")
    except Exception as e:
        print(f"❌ ОШИБКА ГАРМИН (БАН 429?): {e}")
        raise
