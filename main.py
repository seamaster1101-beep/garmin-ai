...
def main():
    ...
    if not today_acts:
        ...
        prompt = (
            ...
            f"\nЖЁСТКИЕ ПРАВИЛА:\n"
            f"- Не делай общих фраз.\n"
            f"- Обязательно используй в ответе числа из данных: IF, среднюю мощность, пульс или TSS.\n"
            f"- Если IF >= 0.90, называй работу высокой интенсивности, пороговой или близкой к пороговой.\n"
            f"- Если средняя мощность >= 90% FTP при длительности от 20 минут, не называй тренировку умеренной.\n"
            f"- Короткая длительность не снижает оценку, если мощность и пульс высокие.\n"
            f"- После интенсивной велоработы не предлагай 'прогулку' как основной совет. "
            f"Базовый совет: восстановительное кручение Z1 20-40 мин или отдых.\n"
            f"- Если это силовая, различай локально тяжёлую мышечную работу и общий кардио-стресс.\n"
            f"- Каждый пункт максимум 2 коротких предложения.\n"
            f"- Без воды. Без шаблонов. Без мотивационной банальщины.\n"

            f"\nОТВЕТЬ СТРОГО ПО ПУНКТАМ:\n"
            f"1. СТАТУС: Назови реальную интенсивность по цифрам.\n"
            f"2. ФИДБЕК: Что именно получилось хорошо по цифрам.\n"
            f"3. ЗАВТРА: Только один конкретный вариант — отдых / Z1 / Z2.\n"

            f"В конце одна короткая фраза на русском."
        )
        fallback_text = (
            f"1. СТАТУС: Работа была высокой интенсивности: IF {if_val}, "
            f"средняя мощность {w_avg} Вт, пульс {hr_avg}/{hr_max_act}.\n"
            f"2. ФИДБЕК: За {dur_min} мин получилась короткая, но реально рабочая тренировка, "
            f"а не лёгкая сессия.\n"
            f"3. ЗАВТРА: Либо отдых, либо восстановительное кручение Z1 20-40 мин без нагрузки."
        )
        ai_msg = ask_arnie(prompt, fallback_text)
