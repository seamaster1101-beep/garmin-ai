...
def main():
    ...
    # Ищем в данных Garmin (sum_data) продолжительность сна в часах
    sleep_hours = sum_data.get("sleep_hours", 0) 
    if sleep_hours == 0:
        # Если ключ называется иначе, попробуй вытащить из общей длительности (обычно в секундах)
        sleep_hours = sum_data.get("duration", 0) / 3600
    ...
    if not today_acts:
        sleep_note = ""
        if sleep < 6.5:
            sleep_note = "- ВАЖНО: Сон < 6.5ч. Жестко снижай интенсивность, запрети агрессивную Зону 3 (Z3).\n"
        prompt = (
            f"Ты — АРНИ, твой стиль: жесткий, лаконичный, без соплей, как тренер элиты. "
            ...
            f"\nИНСТРУКЦИИ: "
            f"{sleep_note}"
            f"Правила контроля зон:\n"
            f"- Если готовность < 2 -> только отдых или лёгкая активность.\n"
            f"- Если готовность от 2 до 3 -> только Z1–Z2.\n"
            f"- Если готовность > 3.5 -> можно Z3.\n"
            f"\n"
            f"Жесткие ограничения:\n"
            f"- Не допускай противоречий: если есть риск для ЦНС — не давай тяжёлую работу.\n"
            f"- Каждый пункт ответа — максимум 3 коротких предложения."
