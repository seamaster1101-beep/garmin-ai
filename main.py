...
def main():
    ...
    if now_dt.hour > 18 or (now_dt.hour == 18 and now_dt.minute >= 30):
        ...
        prompt = (
            ...
            f"- Если суммарный TSS дня около 50-70, но HRV, пульс, Recovery и субъективные оценки хорошие, не называй день тяжёлым или сложным.\n"
            f"- Для такого дня используй формулировки: плотный, полноценный, рабочий, но контролируемый день.\n"
            f"- Если Strength Feel >= 5 или Ride Feel >= 4 при умеренном TSS, подчёркивай хорошую переносимость нагрузки, а не борьбу с ней.\n"
            f"- Не пиши 'нагрузка была сложной', если объективные и субъективные метрики показывают уверенное выполнение без признаков перегруза.\n"
    ...
    if not today_acts:
        ...
        prompt = (
            ...
            f"- Если Recovery низкий, TSB около нуля и пульс/HRV выглядят ровно, не используй тяжёлые формулировки вроде 'значительный дефицит ресурса', 'не перегружать нервную систему' или 'организм не восстановился'.\n"
            f"- В такой ситуации пиши спокойнее: состояние по тренировочной нагрузке ровное, но из-за недосыпа общий ресурс дня снижен.\n"
            f"- В вердикте используй формулировки: лёгкая работа, не добавлять лишний стресс, без Z3 и без интенсивных интервалов.\n"
            ...
            f"- Если Recovery низкий, TSB около нуля и HRV не ниже тренда, не описывай состояние как недовосстановление после нагрузки.\n"
            f"- В таком случае причина ограничения дня — короткий сон и сниженный общий ресурс, а не тренировочная перегрузка.\n"
            f"- Используй формулировки: по нагрузке состояние ровное, но из-за сна сегодня нужен щадящий режим.\n"
            ...
            f"- Если HRV близок к личному среднему, не называй его нижней частью диапазона без явной причины. Используй: HRV в рабочем диапазоне, близко к личной норме, стабильно относительно тренда.\n"
            f"- Если Recovery низкий, TSB около нуля и пульс хороший, не используй формулировки вроде 'недовосстановление после нагрузки' только из-за плохого сна.\n"
            f"- В таком случае пиши точнее: тренировочное восстановление в целом нормальное, но недосып снижает общий ресурс дня и ограничивает готовность.\n"
            f"- В вердикте после плохого сна используй спокойные формулировки: лёгкая работа, Z1 или очень осторожная Z2, сегодня без Z3 и без интенсивных интервалов.\n"
