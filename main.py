...
# --- 1. DAILY (Калории из основного счетчика) ---
try:
    # Самый надежный способ для шагов и дистанции
    ...
    # КАЛОРИИ: Берем через get_stats — там они обновляются мгновенно
    st = gar.get_stats(today_date) or {}
    cals = st.get("calories", "")
    
    # Если в stats пусто, пробуем через summary
    if not cals or cals == 0:
        summary = gar.get_user_summary(today_date) or {}
        cals = summary.get("totalCalories") or (summary.get("activeCalories", 0) + summary.get("bmrCalories", 0))

    r_hr = st.get("restingHeartRate") or ""
    bb = st.get("bodyBatteryMostRecentValue") or ""
...
# --- 3. ACTIVITIES (Глубокий поиск каденса) ---
activities_to_log = []
try:
    ...
    for a in acts:
        ...
        # МАГИЯ КАДЕНСА: проверяем все ключи API
        # Для велодатчиков это обычно 'averageBikingCadence' или 'averageCadence'
...
try:
    ...
    # Activities с защитой от дублей
    act_sheet = ss.worksheet("Activities")
    existing_keys = [f"{r[0]}_{r[1]}_{r[2]}" for r in act_sheet.get_all_values() if len(r) > 2]
    for act in activities_to_log:
        if f"{act[0]}_{act[1]}_{act[2]}" not in existing_keys:
            act_sheet.append_row(act)

    # AI Совет (Универсальный выбор модели)
    advice = "AI Skip"
    if GEMINI_API_KEY:
        try:
    ...
    print(f"✔ Готово. Ккал: {cals}, Каденс: {cad if 'cad' in locals() else '?'}")
