...
if activities_to_log:
    ...
    prompt = (
        ...
        f"\n1. Заголовок: **Утренний Отчет о Состоянии Атлета**"
        f"\n2. Дата: **{display_date}**"
        f"\n3. Тема: Оценка утренней готовности к тренировочному дню."
        f"\n4. Используй жирные заголовки разделов (**1. Сердечно-сосудистая система:** и т.д.)."
        f"\n5. В конце подпись: С уважением, **АРНИ** (Твой Элитный Спортивный Директор)."
...
try:
    ...
    if ai_advice and ai_advice != "SKIP":
        clean_ai = ai_advice.strip() # Убрали замену звездочек, чтобы Markdown работал
