...
def estimate_vo2max(activities, weight=88.0):
    ...
    vals = []
    HR_MAX = 175
    # Защита от кривого веса (чтобы не было деления на 0)
    safe_weight = weight if 50 < weight < 150 else 88.0
    ...
    for a in acts:
        ...
        if w and hr and float(hr) > 100:
            try:
                ...
                v = (10.51 * (w_f * (HR_MAX / hr_f)) / safe_weight) + 7
                # Ослабили фильтр до 75
                if 20 < v < 75: vals.append(v)
            except: continue
    if vals:
        print(f"DEBUG: VO2 candidates: {[round(v,1) for v in vals]}")
...
def fitness_age(rhr, hrv, vo2=None, fat=18.3):
    ...
    try:
        ...
        # Fallback если VO2 нет: считаем как будто он 35 (базовый)
